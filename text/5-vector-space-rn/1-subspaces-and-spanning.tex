...
\begin{wrapfigure}[4]{l}{5cm} 
	\centering
	\vspace*{-2em}
	\input{5-vector-space-rn/figures/1-subspaces-and-spanning/definition5.1plane}
...
\centering
\input{5-vector-space-rn/figures/1-subspaces-and-spanning/example5.1.1}
...
\centering
\input{5-vector-space-rn/figures/1-subspaces-and-spanning/spanning-sets}
...
It turns out that many important subspaces are best described by giving a spanning set. Here are three examples, beginning with an important spanning set for $\RR^n$ itself. Column $j$ of the $n \times n$ identity matrix $I_{n}$ is denoted $\vect{e}_{j}$ and called the $j$th \textbf{coordinate vector}\index{coordinate vectors}\index{vectors!coordinate vectors} in $\RR^n$, and the set $\{\vect{e}_{1}, \vect{e}_{2}, \dots, \vect{e}_{n}\}$ is called the \textbf{standard basis}\index{standard basis} of $\RR^n$. If 
$ \vect{x} = 
	\leftB \begin{array}{r}
	x_1 \\
	x_2 \\
